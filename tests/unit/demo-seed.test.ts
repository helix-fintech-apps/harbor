...
const seeded = [
  ...sql.matchAll(/\('([0-9a-f-]{36})'::uuid, '([^']+@harbor\.test)',\s*'([^']+)'\)/g),
].map(([, id, email, legalName]) => ({ id, email, legalName }));

describe("supabase/seed.sql mirrors the demo-mode users", () => {
  it("seeds every demo user with the same id, email and legal name", () => {
    expect(seeded).toEqual(
      DEMO_USERS.map(({ id, email, legalName }) => ({ id, email, legalName })),
    );
  });
  ...
  it("grants exactly the demo staff roles", () => {
    const grants = [
      ...sql.matchAll(/update public\.profiles set role = '(\w+)' where id = '([0-9a-f-]{36})'/g),
    ].map(([, role, id]) => ({ id, role }));
    expect(grants).toEqual(
      DEMO_USERS.filter((u) => u.role !== "customer").map(({ id, role }) => ({ id, role })),
    );
  });
});
