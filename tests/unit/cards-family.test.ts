...
const req = (amountCents: number, mcc = "5411", foreign = false) => ({
  amountCents,
  mcc,
  merchant: "Shop",
  foreign,
  ...
  it("issuance requires KYC and caps virtual cards / one physical", () => {
    ...
    expect(
      canIssueCard(
        "approved",
        "virtual",
        [...three.slice(0, 2), card({ id: "x", status: "canceled" })],
        P,
      ).ok,
    ).toBe(true);
    expect(
      canIssueCard("approved", "physical", [card({ kind: "physical", status: "requested" })], P)
        .reason,
    ).toBe("physical_card_exists");
  });
});
...
describe("authorizations", () => {
  ...
  it("declines frozen, canceled, replaced, not-yet-activated cards and unapproved owners", () => {
    expect(reason(authorize(req(100), ctx({ card: card({ status: "frozen" }) }), P, F))).toBe(
      "card_frozen",
    );
    expect(reason(authorize(req(100), ctx({ card: card({ status: "canceled" }) }), P, F))).toBe(
      "card_canceled",
    );
    expect(reason(authorize(req(100), ctx({ card: card({ status: "replaced" }) }), P, F))).toBe(
      "card_canceled",
    );
    expect(reason(authorize(req(100), ctx({ card: card({ status: "requested" }) }), P, F))).toBe(
      "card_inactive",
    );
    expect(reason(authorize(req(100), ctx({ ownerKyc: "suspended" }), P, F))).toBe(
      "kyc_not_approved",
    );
    expect(reason(authorize(req(0), ctx(), P, F))).toBe("invalid_amount");
    expect(reason(authorize(req(100), ctx({ accountStatus: "frozen" }), P, F))).toBe(
      "account_frozen",
    );
  });
  it("foreign transactions add 3% to the hold and must fit the balance", () => {
    ...
    expect(
      reason(authorize(req(10_000, "5411", true), ctx({ availableCents: 10_299 }), P, F)),
  ...
  it("tier card-spend limit applies across the owner's cards", () => {
    const used = [
      { at: T("2026-09-24T01:00:00Z"), amountCents: 199_000, kind: "card_spend" as const },
    ];
    expect(
      reason(authorize(req(1_001), ctx({ ownerUsage: used, availableCents: 1e6 }), P, F)),
    ).toBe("daily_limit");
    expect(
      reason(authorize(req(1_000), ctx({ ownerUsage: used, availableCents: 1e6 }), P, F)),
    ).toBe("approved");
  });
});
...
describe("capture / expiry / refunds", () => {
  const auth = (over: Partial<Authorization> = {}): Authorization => ({
    ...
    foreign: false,
    status: "authorized",
    expiresAt: T("2026-10-01T18:00:00Z"),
    funding: { account: "customer_deposits", party: "chk" },
    ...over,
  });
  ...
  it("fuel pre-auth can capture up to the fuel max", () => {
    ...
    expect(
      planCapture(auth({ mcc: "5542", amountCents: 100 }), 8_734, now, P, F).capturedCents,
    ).toBe(8_734);
...
describe("family cards", () => {
  ...
  const teen = (): FamilyMember => ({
    ...newFamilyMember(
      { id: "m1", ownerUserId: "u1", name: "Tia", kind: "teen", limits, existingCount: 0 },
      P,
    ),
    status: "active",
  });
  const spouse = (): FamilyMember =>
    newFamilyMember(
      { id: "m2", ownerUserId: "u1", name: "Sam", kind: "spouse", limits, existingCount: 0 },
      P,
    );
  it("teen requires guardian approval by the owner only; spouse is active immediately", () => {
    const t = newFamilyMember(
      { id: "m1", ownerUserId: "u1", name: "Tia", kind: "teen", limits, existingCount: 0 },
      P,
    );
    ...
    expect(() =>
      newFamilyMember(
        { id: "x", ownerUserId: "u1", name: "X", kind: "spouse", limits, existingCount: 5 },
        P,
      ),
    ).toThrow();
  });
  it("validates limit ordering", () => {
    expect(validateLimits({ perTxnCents: 200, dailyCents: 100, monthlyCents: 1_000 })).toContain(
      "perTxn cannot exceed daily",
    );
    expect(validateLimits({ perTxnCents: 100, dailyCents: 2_000, monthlyCents: 1_000 })).toContain(
      "daily cannot exceed monthly",
    );
  });
  ...
  it("enforces per-txn, daily and monthly member limits (inclusive)", () => {
    ...
    const month = [
      { at: T("2026-09-02T02:00:00Z"), amountCents: 48_000, kind: "card_spend" as const },
    ];
  ...
  it("teen card spends only from the allowance and can't exceed it", () => {
    const m = teen();
    const c = ctx({
      card: card({ familyMemberId: "m1", holderUserId: "u-teen" }),
      member: m,
      memberSpend: [],
      allowanceAvailableCents: 3_000,
      availableCents: 1e6,
    });
    ...
    expect(
      reason(
        authorize(req(100), { ...c, member: { ...m, status: "pending_guardian_approval" } }, P, F),
      ),
    ).toBe("member_inactive");
  });
  it("spouse card spends from the owner's checking within member limits", () => {
    const c = ctx({
      card: card({ familyMemberId: "m2" }),
      member: spouse(),
      memberSpend: [],
  ...
  it("allowance top-up moves owner funds into the teen pocket", () => {
    const t = planAllowanceTopUp({
      id: "tu",
      ownerAccountId: "chk",
      member: teen(),
      amountCents: 2_000,
      ownerAvailableCents: 2_000,
    });
