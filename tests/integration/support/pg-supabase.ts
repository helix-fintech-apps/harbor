...
pg.types.setTypeParser(pg.types.builtins.TIMESTAMPTZ, (v) => (parseTimestamptz(v) as Date).toISOString());
pg.types.setTypeParser(pg.types.builtins.DATE, (v) => v);

type Row = Record<string, unknown>;
export interface PgResult<T = unknown> { data: T; error: { message: string; details?: string; code?: string } | null }
...
class Query implements PromiseLike<PgResult<any>> {
  ...
  constructor(private db: PgSupabase, private table: string, private op: "select" | "insert" | "update", private payload?: Row) {}

  select(columns = "*") { if (this.op === "select") this.columns = columns; return this; }
  single() { this.wantSingle = true; return this; }
  match(m: Row) { for (const [k, v] of Object.entries(m)) this.filters.push([k, v]); return this; }
  ...
  then<A = PgResult<any>, B = never>(ok?: ((v: PgResult<any>) => A | PromiseLike<A>) | null, err?: ((e: unknown) => B | PromiseLike<B>) | null): PromiseLike<A | B> {
    return this.run().then(ok, err);
  }

  private where(params: unknown[], prefix = ""): string {
    if (!this.filters.length) return "";
    return " where " + this.filters.map(([k, v]) => {
    ...
    try {
      ...
      if (this.op === "insert") {
        ...
        const vals = cols.map((c) => {
          ...
          params.push(types[c] === "jsonb" && v !== null && v !== undefined ? JSON.stringify(v) : v ?? null);
        ...
        if (this.orderBy) sql += ` order by l.${ident(this.orderBy.col)} ${this.orderBy.asc ? "asc" : "desc"}`;
        if (this.lim !== undefined) sql += ` limit ${Number(this.lim)}`;
      } else {
        if (this.columns.trim() !== "*") throw new Error(`pg-supabase: unsupported select "${this.columns}"`);
        sql = `select * from ${ident(this.table)}${this.where(params)}`;
        if (this.orderBy) sql += ` order by ${ident(this.orderBy.col)} ${this.orderBy.asc ? "asc" : "desc"}`;
        if (this.lim !== undefined) sql += ` limit ${Number(this.lim)}`;
      }
      const r = await this.db.pool.query(sql, params);
      if (this.op === "update") return { data: null, error: null };
      if (this.wantSingle) {
        if (r.rows.length !== 1) return { data: null, error: { message: `expected 1 row, got ${r.rows.length}` } };
...
export class PgSupabase {
  ...
  async columnTypes(table: string): Promise<Record<string, string>> {
    ...
    const r = await this.pool.query("select column_name, udt_name from information_schema.columns where table_schema = 'public' and table_name = $1", [table]);
    const t: Record<string, string> = Object.fromEntries(r.rows.map((x) => [x.column_name, x.udt_name]));
