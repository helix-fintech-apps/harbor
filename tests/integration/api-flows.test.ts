...
describe("seeded onboarding", () => {
  ...
  it("admin approves a needs_review customer and accounts open; customers can't use admin endpoints", async () => {
    expect((await app.call(as(AVA), "POST", `/admin/users/${RITA.id}/kyc`, { state: "approved", reason: "docs ok" })).status).toBe(403);
    const r = await app.call(as(ADMIN), "POST", `/admin/users/${RITA.id}/kyc`, { state: "approved", reason: "docs ok" });
...
describe("balances and ACH in", () => {
  it("settled deposit is available; new deposit is posted but held", async () => {
    ...
    const r = await app.call(as(AVA), "POST", "/transfers/ach-in", { bankId: bank.id, amountCents: 10_000 });
    expect(r.status).toBe(200);
  ...
  it("R10 return after settlement claws back into a negative balance and blocks closure", async () => {
    ...
    await app.call(as(BEN), "POST", "/transfers/p2p", { recipientEmail: "ava@harbor.test", amountCents: 40_000, stepUpCode: "000000" });
    const r = await app.call(as(ADMIN), "POST", `/admin/transfers/${dep.id}/return`, { code: "R10" });
    expect(body(r).negativeBalanceCents).toBe(40_000);
    expect((await checking(BEN)).postedCents).toBe(-40_000);
    const again = await app.call(as(ADMIN), "POST", `/admin/transfers/${dep.id}/return`, { code: "R10" });
  ...
  it("name mismatch bank is linked but can't be used", async () => {
    const link = body(await app.call(as(AVA), "POST", "/banks/exchange", { publicToken: "public-fake-Other_Bank-Mallory_Smith" }));
    expect(link.nameMatched).toBe(false);
    const r = await app.call(as(AVA), "POST", "/transfers/ach-in", { bankId: link.id, amountCents: 1_000 });
...
describe("money out", () => {
  it("instant withdrawal charges the published fee; cooling-off applies to a freshly linked bank", async () => {
    const bank = (await me()).banks[0];
    const r = await app.call(as(AVA), "POST", "/transfers/ach-out", { bankId: bank.id, amountCents: 10_000, speed: "instant" });
    expect(body(r).feeCents).toBe(150);
    expect((await checking()).postedCents).toBe(250_000 - 10_150);
    const fresh = body(await app.call(as(AVA), "POST", "/banks/exchange", { publicToken: "public-fake-New_Bank-Ava_Harbor" }));
    expect(body(await app.call(as(AVA), "POST", "/transfers/ach-out", { bankId: fresh.id, amountCents: 100, speed: "standard" })).error.code).toBe("cooling_off");
  });
  it("idempotent transfers: same key returns the first result and moves money once; different body conflicts", async () => {
    ...
    const a1 = await app.call(as(AVA), "POST", "/transfers/ach-out", b, { idempotencyKey: "k-1" });
    const a2 = await app.call(as(AVA), "POST", "/transfers/ach-out", b, { idempotencyKey: "k-1" });
    expect(body(a2).id).toBe(body(a1).id);
    expect(a2.replayed).toBe(true);
    expect((await checking()).postedCents).toBe(245_000);
    const a3 = await app.call(as(AVA), "POST", "/transfers/ach-out", { ...b, amountCents: 6_000 }, { idempotencyKey: "k-1" });
    expect(body(a3).error.code).toBe("idempotency_conflict");
  });
  it("P2P new payee needs step-up; tier1 daily limit applies", async () => {
    expect(body(await app.call(as(AVA), "POST", "/transfers/p2p", { recipientEmail: "ben@harbor.test", amountCents: 1_000 })).error.code).toBe("step_up_required");
    expect(body(await app.call(as(AVA), "POST", "/transfers/p2p", { recipientEmail: "ben@harbor.test", amountCents: 1_000, stepUpCode: "123456" })).error.code).toBe("step_up_required");
    expect((await app.call(as(AVA), "POST", "/transfers/p2p", { recipientEmail: "ben@harbor.test", amountCents: 1_000, stepUpCode: "000000" })).status).toBe(200);
    expect((await app.call(as(AVA), "POST", "/transfers/p2p", { recipientEmail: "ben@harbor.test", amountCents: 99_000 })).status).toBe(200);
    expect(body(await app.call(as(AVA), "POST", "/transfers/p2p", { recipientEmail: "ben@harbor.test", amountCents: 1 })).error.code).toBe("below_minimum");
    expect(body(await app.call(as(AVA), "POST", "/transfers/p2p", { recipientEmail: "ben@harbor.test", amountCents: 100 })).error.code).toBe("daily_limit");
    expect(body(await app.call(as(AVA), "POST", "/transfers/p2p", { recipientEmail: "rita@harbor.test", amountCents: 100 })).error.code).toBe("recipient_unavailable");
  });
});
...
describe("cards", () => {
  it("auth holds available, partial capture releases the rest, refund posts once", async () => {
    ...
    const cap = body(await app.call(as(AVA), "POST", `/sim/authorizations/${a.authorizationId}/capture`, { amountCents: 5_900 }));
    expect(cap.capturedCents).toBe(5_900); // tip within 20%
    const chk = await checking();
    expect([chk.postedCents, chk.availableCents]).toEqual([244_100, 244_100]);
    const r1 = body(await app.call(as(AVA), "POST", `/sim/authorizations/${a.authorizationId}/refund`, { refundId: "re_1", amountCents: 900 }));
    const r2 = body(await app.call(as(AVA), "POST", `/sim/authorizations/${a.authorizationId}/refund`, { refundId: "re_1", amountCents: 900 }));
  ...
  it("frozen card declines; auth expiry releases the hold", async () => {
    ...
    expect(body(await app.call(as(AVA), "POST", `/sim/cards/${card.id}/authorize`, { amountCents: 100, mcc: "5411" })).reason).toBe("card_frozen");
    await app.call(as(AVA), "POST", `/cards/${card.id}/unfreeze`);
    await app.call(as(AVA), "POST", `/sim/cards/${card.id}/authorize`, { amountCents: 7_000, mcc: "5411" });
  ...
  it("teen family card: guardian approval, allowance only, MCC block", async () => {
    const m = body(await app.call(as(AVA), "POST", "/family", { name: "Tia", kind: "teen", limits: { perTxnCents: 5_000, dailyCents: 10_000, monthlyCents: 40_000 } }));
    expect(m.status).toBe("pending_guardian_approval");
    const card = body(await app.call(as(AVA), "POST", "/cards", { kind: "virtual", familyMemberId: m.id }));
    expect(body(await app.call(as(AVA), "POST", `/sim/cards/${card.id}/authorize`, { amountCents: 100, mcc: "5411" })).reason).toBe("member_inactive");
    ...
    expect(body(await app.call(as(AVA), "POST", `/sim/cards/${card.id}/authorize`, { amountCents: 2_001, mcc: "5411" })).reason).toBe("allowance_exceeded");
    expect(body(await app.call(as(AVA), "POST", `/sim/cards/${card.id}/authorize`, { amountCents: 500, mcc: "7995" })).reason).toBe("mcc_blocked");
    expect(body(await app.call(as(AVA), "POST", `/sim/cards/${card.id}/authorize`, { amountCents: 2_000, mcc: "5411" })).approved).toBe(true);
...
describe("disputes, interest, statements, closure", () => {
  it("dispute -> provisional credit -> lost reverses", async () => {
    ...
    const d = body(await app.call(as(AVA), "POST", "/disputes", { authorizationId: a.authorizationId, amountCents: 3_000, reason: "not received" }));
    expect(d.status).toBe("open");
    expect(body(await app.call(as(AVA), "POST", "/disputes", { authorizationId: a.authorizationId, amountCents: 1, reason: "x" })).error.code).toBe("dispute_rejected");
  ...
  it("savings interest accrues daily and posts monthly with banker's rounding", async () => {
    await app.call(as(AVA), "POST", "/transfers/pocket", { from: "checking", to: "savings", amountCents: 100_000 });
    const day0 = app.clock.now();
    for (let i = 0; i < 3; i++) {
      await app.call(as(ADMIN), "POST", "/admin/jobs/accrue-interest", { day: new Date(day0.getTime() + i * 86_400_000).toISOString().slice(0, 10) });
    }
  ...
  it("statement reconciles with the ledger", async () => {
    const chk = await checking();
    const s = body(await app.call(as(AVA), "GET", `/statements?accountId=${chk.id}&period=2026-09`));
    expect(s.openingCents).toBe(0);
    expect(s.closingCents).toBe(chk.postedCents);
    expect(body(await app.call(as(BEN), "GET", `/statements?accountId=${chk.id}&period=2026-09`)).error.code).toBe("not_found");
  });
...
describe("providers", () => {
  it("refuses live keys and uses fakes without keys", () => {
    ...
    expect(selectProviders({ STRIPE_SECRET_KEY: "sk_test_x" }).identity.name).toBe("stripe_identity");
  });
});
