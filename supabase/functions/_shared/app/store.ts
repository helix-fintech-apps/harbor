...
export interface LedgerPost extends Txn {
  idem: string;
}
...
export interface ClosureExpectation {
  accounts: { id: string; postedCents: number }[];
  members: { id: string; postedCents: number }[];
}
...
export function netDebit(
  t: Txn | null | undefined,
  account: LedgerAccount,
  party: string | null,
): number {
  let n = 0;
  for (const l of t?.lines ?? [])
    if (l.account === account && (l.party ?? null) === party) n += l.debit - l.credit;
  return n;
}

const fail = (code: MoneyOpCode, message?: string): never => {
  throw new MoneyOpError(code, message);
};
...
export class MemoryStore implements Store {
  ...
  private post(t: LedgerPost, at: string): string {
    ...
    try {
      ...
      fail("unbalanced_ledger", (e as Error).message);
    }
    const id = uuid();
    this.txns.push({
      id,
  ...
  private posted(account: "customer_deposits" | "family_allowance", party: string): number {
    ...
    for (const l of this.lines)
      if (l.account === account && l.party === party) b += l.credit - l.debit;
    return b;
  }
  ...
  private activeHolds(pred: (h: Row) => boolean, at: string): number {
    ...
    for (const h of this.t("holds")) {
      if (
        h.status === "active" &&
        (!h.expires_at || new Date(h.expires_at).getTime() > now) &&
        pred(h)
      )
        s += Number(h.amount_cents);
    }
    return s;
  }
  ...
  private available(accountId: string, at: string): number {
    return (
      this.posted("customer_deposits", accountId) -
      this.activeHolds((h) => h.account_id === accountId, at)
    );
  }

  private allowanceAvailable(memberId: string, at: string): number {
    return (
      this.posted("family_allowance", memberId) -
      this.activeHolds((h) => h.account_id == null && h.family_member_id === memberId, at)
    );
  }
  ...
  private checkLimit(userId: string, limit: LimitWindow | null, amount: number, at: string) {
    ...
    const now = new Date(at).getTime(),
      day = limit.dayStart.getTime(),
      month = limit.monthStart.getTime();
    let today = 0,
      mon = 0;
    ...
    if (today + amount > limit.dailyCents)
      fail("daily_limit", `daily limit ${limit.dailyCents}: used ${today}, requested ${amount}`);
    if (mon + amount > limit.monthlyCents)
      fail(
        "monthly_limit",
        `monthly limit ${limit.monthlyCents}: used ${mon}, requested ${amount}`,
      );
  }
  ...
  async ledger(filter: { account?: string; party?: string; limit?: number } = {}) {
    ...
    const out = this.lines
      .filter(
        (l) =>
          (!filter.account || l.account === filter.account) &&
          (!filter.party || l.party === filter.party),
      )
      .map((l) => {
        const t = byId.get(l.txn_id)!;
        return { ...l, kind: t.kind, ref: t.ref, at: t.created_at };
      });
    return filter.limit ? out.slice(-filter.limit) : out;
  }
  toJSON() {
    return { tables: this.tables, txns: this.txns, lines: this.lines };
  }
  load(data: {
    tables: Record<string, Row[]>;
    txns: MemoryStore["txns"];
    lines: MemoryStore["lines"];
  }) {
  ...
  private transferOut(
    ...
    if (transfer.kind !== kind)
      fail("payload_mismatch", `expected a ${kind} transfer, got ${transfer.kind}`);
    ...
    if (debited !== need)
      fail(
        "payload_mismatch",
        `ledger debits ${debited} from the source account, transfer amount + fee is ${need}`,
      );
    if (!this.row("accounts", { id: transfer.from_account_id }))
      fail("not_found", "source account not found");
  ...
  achPullCreate(p: {
    ...
    return this.tx(() => {
      ...
      if (
        netDebit(p.ledger, "customer_deposits", t.to_account_id) !== -amount ||
        Number(hold.amount_cents) !== amount ||
        hold.ref_id !== t.id ||
        hold.account_id !== t.to_account_id
      ) {
        fail("payload_mismatch", "ACH pull credit, deposit hold and transfer must agree");
      }
      if (!this.row("accounts", { id: t.to_account_id }))
        fail("not_found", "destination account not found");
  ...
  achSettle(p: { transferId: string; at: string }) {
    return this.tx(() => {
      ...
      if (!t || !t.settle_at || new Date(t.settle_at).getTime() > new Date(p.at).getTime())
        return false;
      Object.assign(t, { status: "settled", settled_at: p.at });
      this.patch(
        "holds",
        { ref_id: p.transferId, status: "active" },
        { status: "released", released_at: p.at },
      );
      return true;
    });
  }
  ...
  achReturn(p: {
    ...
    return this.tx(() => {
      ...
      if (netDebit(p.ledger, "customer_deposits", t.to_account_id) !== Number(t.amount_cents))
        fail("payload_mismatch", "a return reverses exactly the deposited amount");
      this.post(p.ledger, p.at);
      const releasedHolds = this.patch(
        "holds",
        { ref_id: p.transferId, status: "active" },
        { status: "released", released_at: p.at },
      );
  ...
  achPush(p: { transfer: Row; ledger: LedgerPost; limit: LimitWindow | null; at: string }) {
    return this.tx(() => {
      if (netDebit(p.ledger, "fee_revenue", null) !== -Number(p.transfer.fee_cents ?? 0))
        fail("payload_mismatch", "fee revenue must equal the transfer fee");
  ...
  p2pTransfer(p: {
    ...
    return this.tx(() => {
      if (
        netDebit(p.ledger, "customer_deposits", p.transfer.to_account_id) !==
        -Number(p.transfer.amount_cents)
      )
        fail("payload_mismatch", "recipient must be credited the transfer amount");
      const r = this.transferOut("p2p", p.transfer, p.ledger, p.limit, p.at);
      if (
        p.payee &&
        !r.replayed &&
        !this.row("payees", { user_id: p.payee.user_id, payee_user_id: p.payee.payee_user_id })
      ) {
  ...
  pocketMove(p: { transfer: Row; ledger: LedgerPost; at: string }) {
    return this.tx(() => {
      if (
        ...
        fail("payload_mismatch", "destination pocket must be credited the amount");
  ...
  allowanceTopUp(p: { transfer: Row; ledger: LedgerPost; at: string }) {
    return this.tx(() => {
      if (
        netDebit(p.ledger, "family_allowance", p.transfer.family_member_id) !==
        -Number(p.transfer.amount_cents)
      )
  ...
  cardAuthorize(p: { auth: Row; hold: Row | null; at: string }) {
    return this.tx(() => {
      const { auth, hold } = p;
      const prev = this.t("card_authorizations").find(
        (a) =>
          a.id === auth.id ||
          (auth.provider_auth_id != null && a.provider_auth_id === auth.provider_auth_id),
      );
      if (prev)
        return {
          approved: prev.status !== "declined",
          reason: prev.decline_reason ?? null,
          holdId: prev.hold_id ?? null,
          replayed: true,
        };
      const defaults = {
        fee_cents: 0,
      ...
      if (!hold) {
        if (auth.status !== "declined")
          fail("payload_mismatch", "an approved authorization needs a hold");
      ...
      if (
        need !== Number(auth.amount_cents) + Number(auth.fee_cents ?? 0) ||
        hold.ref_id !== auth.id
      )
        fail("payload_mismatch", "the hold must cover amount + fees of this authorization");
      let avail: number, reason: string;
      if (auth.funding_account === "family_allowance") {
        if (hold.family_member_id !== auth.funding_party || hold.account_id != null)
          fail("payload_mismatch", "allowance holds sit on the member pocket");
        avail = this.allowanceAvailable(auth.funding_party, p.at);
        reason = "allowance_exceeded";
      } else {
        if (hold.account_id !== auth.funding_party)
          fail("payload_mismatch", "card holds sit on the funding account");
        avail = this.available(auth.funding_party, p.at);
        reason = "insufficient_funds";
      }
      if (need > avail) {
        this.put("card_authorizations", {
          ...defaults,
          ...auth,
          status: "declined",
  ...
  cardCapture(p: {
    ...
    return this.tx(() => {
      ...
      if (a.status !== "authorized")
        fail("invalid_state", `cannot capture a ${a.status} authorization`);
      if (new Date(a.expires_at).getTime() <= new Date(p.at).getTime())
        fail("auth_expired", "authorization expired");
      if (
        p.capturedCents <= 0 ||
        p.feeCents < 0 ||
        netDebit(p.ledger, a.funding_account, a.funding_party) !== p.capturedCents + p.feeCents
      ) {
  ...
  cardRefund(p: {
    ...
    return this.tx(() => {
      ...
      if (prev) {
        if (prev.auth_id !== p.authId)
          fail("refund_id_conflict", "refund id already used for another purchase");
      ...
      if (
        p.amountCents <= 0 ||
        Number(a.refunded_cents) + p.amountCents > Number(a.captured_cents)
      ) {
        fail(
          "refund_exceeds_captured",
          `captured ${a.captured_cents}, refunded ${a.refunded_cents}, requested ${p.amountCents}`,
        );
      }
  ...
  disputeOpen(p: { dispute: Row; at: string }) {
    return this.tx(() => {
      ...
      if (a.status !== "captured")
        fail("invalid_state", "only posted (captured) purchases can be disputed");
      if (d.credit_account !== a.funding_account || d.credit_party !== a.funding_party)
        fail("payload_mismatch", "a dispute credits the pocket that paid");
      if (
        this.t("disputes").some(
          (x) => x.auth_id === a.id && (x.status === "open" || x.status === "provisional_credited"),
        )
      ) {
        fail("dispute_already_open", "a dispute is already open for this transaction");
      }
      if (Number(d.amount_cents) > Number(a.captured_cents) - Number(a.refunded_cents))
        fail("dispute_exceeds_amount", "dispute exceeds the unrefunded purchase amount");
      return {
        ...this.put("disputes", {
          status: "open",
  ...
  disputeProvisionalCredit(p: { disputeId: string; ledger: LedgerPost; at: string }) {
    return this.tx(() => {
      ...
      if (d.status !== "open")
        fail("invalid_state", `provisional credit not allowed in ${d.status}`);
      if (netDebit(p.ledger, d.credit_account, d.credit_party) !== -Number(d.amount_cents))
        fail("payload_mismatch", "provisional credit must equal the disputed amount");
      this.post(p.ledger, p.at);
      Object.assign(d, {
        status: "provisional_credited",
        provisional_credit_cents: d.amount_cents,
      });
    });
  }
  ...
  disputeResolve(p: {
    ...
    return this.tx(() => {
      if (p.outcome !== "won" && p.outcome !== "lost")
        fail("payload_mismatch", "outcome must be won or lost");
      const d = this.row("disputes", { id: p.disputeId });
      if (!d) return fail("not_found", "dispute not found");
      if (d.status !== "open" && d.status !== "provisional_credited")
        fail("invalid_state", `dispute already ${d.status}`);
      if (d.status !== p.expectedStatus)
        fail("invalid_state", `dispute moved to ${d.status} while resolving`);
      if (p.ledger) this.post(p.ledger, p.at);
      Object.assign(d, {
        status: p.outcome,
        provisional_credit_cents: p.provisionalCreditCents,
        resolved_at: p.at,
      });
      this.put("audit_log", {
        actor_id: p.actorId,
  ...
  postInterest(p: { posting: Row; ledger: LedgerPost | null; at: string }) {
    return this.tx(() => {
      if (
        this.row("interest_postings", {
          account_id: p.posting.account_id,
          period: p.posting.period,
        })
      )
        return false;
      ...
      if (p.ledger) {
        if (
          netDebit(p.ledger, "customer_deposits", p.posting.account_id) !==
          -Number(p.posting.posted_cents)
        )
          fail("payload_mismatch", "interest ledger must credit posted_cents");
        this.post(p.ledger, p.at);
  ...
  closeAccount(p: {
    ...
    return this.tx(() => {
      ...
      for (const a of accounts) {
        if (a && a.user_id !== p.userId)
          fail("payload_mismatch", "account belongs to another customer");
        if (a?.status === "closed") fail("already_closed", "account already closed");
      }
      if (
        accounts.some((a) => !a) ||
        this.t("accounts").some(
          (a) => a.user_id === p.userId && a.status !== "closed" && !accountIds.includes(a.id),
        )
      ) {
        fail("closure_state_changed", "the set of open pockets changed");
      }
      for (const e of p.expected.accounts)
        if (this.posted("customer_deposits", e.id) !== e.postedCents)
          fail("closure_state_changed", "a pocket balance changed while closing");
      for (const e of p.expected.members)
        if (this.posted("family_allowance", e.id) !== e.postedCents)
          fail("closure_state_changed", "an allowance balance changed while closing");
      if (
        this.activeHolds(
          (h) => accountIds.includes(h.account_id) || memberIds.includes(h.family_member_id),
          p.at,
        ) > 0
      )
        fail("closure_state_changed", "pending holds");
      if (
        this.t("disputes").some(
          (d) =>
            d.user_id === p.userId && (d.status === "open" || d.status === "provisional_credited"),
        )
      )
        fail("closure_state_changed", "open disputes");

      const canceledCardIds: string[] = [];
      for (const c of this.t("cards")) {
        if (
          accountIds.includes(c.account_id) &&
          c.status !== "canceled" &&
          c.status !== "replaced"
        ) {
      ...
      if (
        accountIds.some((id) => this.posted("customer_deposits", id) !== 0) ||
        memberIds.some((id) => this.posted("family_allowance", id) !== 0)
      ) {
        fail("payload_mismatch", "closure payout must leave every pocket at zero");
      }
      for (const id of accountIds)
        this.patch("accounts", { id }, { status: "closed", closed_at: p.at });
...
export class SupabaseStore implements Store {
  ...
  async one(table: string, match: Row) {
    return (
      this.check(await this.client.from(table).select("*").match(match).limit(1)) as Row[]
    )[0];
  }
  async list(table: string, match: Row = {}) {
  ...
  async postLedger(t: Txn, idempotencyKey?: string) {
    assertBalanced(t);
    return this.check(
      await this.client.rpc("post_ledger_txn", {
        p_kind: t.kind,
        p_ref: t.ref ?? null,
  ...
  async ledger(filter: { account?: string; party?: string; limit?: number } = {}) {
    let q = this.client
      .from("ledger_lines")
      .select("txn_id, account, party, debit, credit, ledger_txns(kind, ref, created_at)");
    ...
    return rows.map((r) => ({
      ...
      kind: r.ledger_txns.kind,
      ref: r.ledger_txns.ref,
      at: r.ledger_txns.created_at,
    }));
  }
  ...
  private static limitArg(l: LimitWindow | null) {
    ...
    return {
      kind: l.kind,
      daily_cents: l.dailyCents,
      monthly_cents: l.monthlyCents,
      day_start: l.dayStart.toISOString(),
      month_start: l.monthStart.toISOString(),
    };
  }
  ...
  async achPullCreate(p: {
    ...
    const r = await this.rpc<{ replayed: boolean }>("harbor_ach_pull_create", {
  ...
  async achReturn(p: {
    ...
    const r = await this.rpc<{ released_holds: number }>("harbor_ach_return", {
  ...
  private async transferOut(
    fn: string,
    p: { transfer: Row; ledger: LedgerPost; at: string },
    extra: Record<string, unknown> = {},
  ) {
    const r = await this.rpc<{ replayed: boolean }>(fn, {
      p_transfer: p.transfer,
      p_ledger: SupabaseStore.ledgerArg(p.ledger),
      p_at: p.at,
      ...extra,
  ...
  achPush(p: { transfer: Row; ledger: LedgerPost; limit: LimitWindow | null; at: string }) {
    return this.transferOut("harbor_ach_push", p, { p_limit: SupabaseStore.limitArg(p.limit) });
  }
  p2pTransfer(p: {
    transfer: Row;
    ledger: LedgerPost;
    ...
    return this.transferOut("harbor_p2p_transfer", p, {
      p_payee: p.payee,
      p_limit: SupabaseStore.limitArg(p.limit),
    });
  }
  pocketMove(p: { transfer: Row; ledger: LedgerPost; at: string }) {
    return this.transferOut("harbor_pocket_move", p);
  }
  allowanceTopUp(p: { transfer: Row; ledger: LedgerPost; at: string }) {
  ...
  async cardAuthorize(p: { auth: Row; hold: Row | null; at: string }) {
    const r = await this.rpc<{
      ...
      hold_id: string | null;
      replayed: boolean;
    }>("harbor_card_authorize", { p_auth: p.auth, p_hold: p.hold, p_at: p.at });
    return {
      approved: !!r.approved,
  ...
  async disputeProvisionalCredit(p: { disputeId: string; ledger: LedgerPost; at: string }) {
    await this.rpc("harbor_dispute_provisional_credit", {
      p_dispute_id: p.disputeId,
      p_ledger: SupabaseStore.ledgerArg(p.ledger),
      p_at: p.at,
    });
  }
  async disputeResolve(p: {
    disputeId: string;
    outcome: "won" | "lost";
    expectedStatus: string;
    provisionalCreditCents: number;
    ...
    await this.rpc("harbor_dispute_resolve", {
      ...
      p_provisional_credit_cents: p.provisionalCreditCents,
      p_ledger: SupabaseStore.ledgerArg(p.ledger),
      p_at: p.at,
      p_actor: p.actorId,
    });
  ...
  postInterest(p: { posting: Row; ledger: LedgerPost | null; at: string }) {
    return this.rpc<boolean>("harbor_post_interest", {
      p_posting: p.posting,
      p_ledger: SupabaseStore.ledgerArg(p.ledger),
      p_at: p.at,
    });
  }
  async closeAccount(p: {
    userId: string;
    closure: Row;
    expected: ClosureExpectation;
    ledger: LedgerPost | null;
    ...
    const r = await this.rpc<{ canceled_card_ids: string[] }>("harbor_close_account", {
      ...
      p_expected: expected,
      p_ledger: SupabaseStore.ledgerArg(p.ledger),
      p_payout_transfer: p.payoutTransfer,
      p_at: p.at,
      p_actor: p.actorId,
      p_audit: p.audit,
    });
