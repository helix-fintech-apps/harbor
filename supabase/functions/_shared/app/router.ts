...
export interface ApiRequest { method: string; path: string; body: any; caller: Caller | null; idempotencyKey?: string; query?: Record<string, string> }
export interface ApiResponse { status: number; body: unknown; replayed?: boolean }

type Handler = (s: HarborService, c: Caller, p: Record<string, string>, body: any, q: Record<string, string>) => Promise<unknown>;

interface Route { method: string; pattern: RegExp; keys: string[]; handler: Handler; mutating: boolean; public?: boolean }
...
add("GET", "/transfers/fee-quote", async (s, _c, _p, _b, q) => s.quoteFee(Number(q.amountCents), q.speed === "instant" ? "instant" : "standard"));
...
add("POST", "/family/:id/allowance", (s, c, p, b) => s.allowanceTopUp(c, p.id, b?.amountCents, b?.idempotencyKey));
...
add("POST", "/sim/cards/:id/authorize", async (s, c, p, b) => { await s.cardFor(c, p.id); return s.authorizeCard(p.id, b); });
add("POST", "/sim/authorizations/:id/capture", async (s, _c, p, b) => s.capture(p.id, b?.amountCents));
add("POST", "/sim/authorizations/:id/refund", async (s, _c, p, b) => s.merchantRefund(p.id, String(b?.refundId ?? ""), b?.amountCents));
...
add("POST", "/admin/accounts/:id/freeze", (s, c, p, b) => s.adminSetAccountStatus(c, p.id, "frozen", b?.reason));
add("POST", "/admin/accounts/:id/unfreeze", (s, c, p, b) => s.adminSetAccountStatus(c, p.id, "open", b?.reason));
...
export async function route(s: HarborService, req: ApiRequest): Promise<ApiResponse> {
  ...
  try {
    for (const r of routes) {
      ...
      if (r.mutating) {
        const res = await s.idempotent(caller, req.idempotencyKey, `${req.method} ${path}`, req.body ?? null,
          () => r.handler(s, caller, params, { ...(req.body ?? {}), idempotencyKey: req.idempotencyKey }, req.query ?? {}));
