...
export class FakeBankLink implements BankLinkProvider {
  ...
  async createLinkToken(userId: string) {
    return `link-sandbox-fake-${userId.slice(0, 8)}`;
  }
  async exchangePublicToken(publicToken: string) {
    if (!publicToken.startsWith("public-fake-")) throw new Error("invalid public token");
    return {
      itemId: `item-${publicToken.slice(12)}`,
      accessToken: `access-fake-${publicToken.slice(12)}`,
    };
  }
...
export class PlaidSandbox implements BankLinkProvider {
  ...
  async getAccount(accessToken: string): Promise<BankAccountInfo> {
    ...
    const ident = await this.call("/identity/get", {
      access_token: accessToken,
      options: { account_ids: [acct.account_id] },
    });
    ...
    return {
      ...
      mask: acct.mask ?? "0000",
      name: acct.name,
      institution: auth.item?.institution_id ?? "Plaid Sandbox",
      ownerNames: owners,
    };
  }
}
