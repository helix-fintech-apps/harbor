...
export function assertNoLiveKeys(env: Env): void {
  if (env.STRIPE_SECRET_KEY?.startsWith("sk_live_") || env.STRIPE_SECRET_KEY?.startsWith("rk_live_")) {
    throw new LiveKeyRefused("Refusing to run with a live Stripe key. Harbor is a test subject: use sk_test_ keys only.");
  }
  if (env.PLAID_ENV && env.PLAID_ENV !== "sandbox") {
    throw new LiveKeyRefused(`Refusing PLAID_ENV=${env.PLAID_ENV}. Only the Plaid sandbox is allowed.`);
  }
}
...
export async function stripeRequest(env: Env, method: "GET" | "POST", path: string, form?: Record<string, string>): Promise<any> {
