...
export function mapIdentityStatus(raw: string | null | undefined): IdentityOutcome {
  switch (raw) {
    ...
    case "requires_input":
      return { kind: "requires_input" };
    case "processing":
      return { kind: "processing" };
    case "canceled":
    case "failed":
      return { kind: "failed", reason: raw };
    case "timeout":
      return { kind: "timeout" };
    default:
...
export function screenSanctions(
  fullName: string,
  policy: MoneyPolicy,
  list: string[] = FAKE_SANCTIONS_LIST,
): SanctionsOutcome {
  ...
  if (
    best &&
    best.score >= policy.kyc.sanctionsFuzzyThresholdBps &&
...
export function decideKyc(
  ...
  sanctions: SanctionsOutcome | null,
): { state: KycState; reason: string } {
  if (sanctions?.kind === "confirmed_match")
    return { state: "frozen_legal", reason: `sanctions match: ${sanctions.entry}` };
  switch (identity.kind) {
    case "failed":
      return {
        state: "rejected",
        reason: `identity failed${identity.reason ? `: ${identity.reason}` : ""}`,
      };
    case "requires_input":
      return { state: "needs_review", reason: "identity requires input" };
    case "processing":
      return { state: "pending", reason: "identity processing" };
    case "timeout":
      return { state: "pending", reason: "identity vendor timeout" };
    case "unknown":
      return { state: "pending", reason: `unknown identity status: ${identity.raw}` };
    case "verified":
      break;
  }
  if (sanctions === null || sanctions.kind === "error")
    return { state: "pending", reason: "sanctions screen not completed" };
  if (sanctions.kind === "potential_match")
    return { state: "needs_review", reason: `potential sanctions match: ${sanctions.entry}` };
