...
export function disputeTimeline(
  ...
  policy: MoneyPolicy,
): { provisionalCreditDueAt: Date; resolutionDueAt: Date } {
  const ageDays = (openedAt.getTime() - accountOpenedAt.getTime()) / 86_400_000;
  const newAccount = ageDays < policy.disputes.newAccountDays;
  return {
    provisionalCreditDueAt: addBusinessDays(
      openedAt,
      policy.disputes.provisionalCreditBusinessDays,
      policy.holidays,
    ),
    resolutionDueAt: addDays(
      openedAt,
      newAccount ? policy.disputes.newAccountResolutionDays : policy.disputes.resolutionDays,
    ),
  };
}
...
export function openDispute(
  ...
  if (p.amountCents > p.capturedCents - p.refundedCents)
    throw new Error("dispute exceeds the unrefunded purchase amount");
  if (p.now.getTime() > addDays(p.postedAt, policy.disputes.windowDays).getTime())
    throw new Error("dispute window closed");
  const tl = disputeTimeline(p.now, p.accountOpenedAt, policy);
  return {
    id: p.id,
    authId: p.authId,
    accountId: p.accountId,
    creditAccount: p.creditAccount ?? "customer_deposits",
...
export function planProvisionalCredit(d: Dispute): { dispute: Dispute; ledger: Txn } {
  ...
  return {
    ...
    ledger: txn(
      ...
      [dr("dispute_receivable", d.amountCents), cr(d.creditAccount, d.amountCents, d.accountId)],
      d.id,
    ),
  };
}
...
export function resolveDispute(
  d: Dispute,
  outcome: "won" | "lost",
): { dispute: Dispute; ledger?: Txn } {
  if (d.status !== "open" && d.status !== "provisional_credited")
    throw new Error(`dispute already ${d.status}`);
  ...
  return {
    ...
    ledger: txn(
      "dispute_lost_reversal",
      [dr(d.creditAccount, d.amountCents, d.accountId), cr("dispute_receivable", d.amountCents)],
      d.id,
    ),
  };
}
