...
export function newFamilyMember(
  ...
  return {
    ...
    kind: p.kind,
    status:
      teen && policy.family.teenRequiresGuardianApproval ? "pending_guardian_approval" : "active",
    limits: p.limits,
    blockedMccGroups: [...blocks],
    blockedMccs: [],
  };
}
...
export function guardianApprove(m: FamilyMember, approverUserId: string): FamilyMember {
  ...
  if (approverUserId !== m.ownerUserId)
    throw new Error("only the guardian (account owner) can approve");
...
export function checkFamilySpend(
  m: FamilyMember,
  mcc: string,
  amountCents: number,
  spend: UsageEvent[],
  ...
  const day = startOfUtcDay(now).getTime(),
    month = startOfUtcMonth(now).getTime();
  let today = 0,
    mon = 0;
  ...
  if (mon + amountCents > m.limits.monthlyCents)
    return { ok: false, reason: "member_monthly_limit" };
  return { ok: true };
}

/** Owner tops up a teen's allowance pocket from checking. */
export function planAllowanceTopUp(p: {
  id: string;
  ownerAccountId: string;
  member: FamilyMember;
  amountCents: number;
  ownerAvailableCents: number;
}): Txn {
  ...
  return txn(
    "allowance_topup",
    [
      dr("customer_deposits", p.amountCents, p.ownerAccountId),
      cr("family_allowance", p.amountCents, p.member.id),
    ],
    p.id,
  );
}
