...
export function ownerNameMatches(legalName: string, ownerNames: string[]): boolean {
  ...
  const first = me[0], last = me[me.length - 1];
...
export interface AchInPlan { settleAt: Date; holdCents: number; ledger: Txn }

/** Plan an ACH pull: credit posts immediately, a hold of the full amount lasts until settlement. */
export function planAchPull(p: {
  transferId: string; accountId: string; amountCents: number; bank: LinkedBank; now: Date;
}, policy: MoneyPolicy): AchInPlan {
  assertPositiveCents(p.amountCents);
  if (p.bank.status !== "active") throw new Error("bank not active");
  if (policy.achIn.nameMatchRequired && !p.bank.nameMatched) throw new Error("account owner name does not match");
  return {
    settleAt: addBusinessDays(p.now, policy.achIn.holdBusinessDays, policy.holidays),
    holdCents: p.amountCents,
    ledger: txn("ach_in", [dr("ach_clearing", p.amountCents), cr("customer_deposits", p.amountCents, p.accountId)], p.transferId),
  };
}
 ...
 */
export function planAchReturn(p: {
  transferId: string; accountId: string; amountCents: number; returnCode: string;
  status: AchInStatus; postedBalanceCents: number;
}, policy: MoneyPolicy): AchReturnPlan {
  ...
  return {
    ...
    ledger: txn(`ach_return_${p.returnCode}`, [dr("customer_deposits", p.amountCents, p.accountId), cr("ach_clearing", p.amountCents)], p.transferId),
  };
}

export interface DirectDepositForm {
  employerName: string;
  allocation: { kind: "full" } | { kind: "percent"; percent: number } | { kind: "fixed"; amountCents: number };
...
export function validateDirectDepositForm(f: DirectDepositForm, legalName: string): string[] {
  ...
  if (f.allocation.kind === "percent" && (!Number.isInteger(f.allocation.percent) || f.allocation.percent < 1 || f.allocation.percent > 100)) errors.push("percent must be 1-100");
  if (f.allocation.kind === "fixed" && (!Number.isInteger(f.allocation.amountCents) || f.allocation.amountCents <= 0)) errors.push("fixed amount must be positive cents");
  ...
  if (normalizeName(f.signatureName).join(" ") !== normalizeName(legalName).join(" ")) errors.push("signature must match legal name");
  return errors;
}
