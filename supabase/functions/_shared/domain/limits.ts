...
export function limitsFor(
  ...
  if (kind === "transfer_out")
    return { daily: t.dailyTransferOutCents, monthly: t.monthlyTransferOutCents };
  if (kind === "card_spend")
    return { daily: t.dailyCardSpendCents, monthly: t.monthlyCardSpendCents };
...
export function checkLimit(
  ...
  if (amountCents > remainingDaily)
    return { ok: false, reason: "daily_limit", remainingDaily, remainingMonthly };
  if (amountCents > remainingMonthly)
    return { ok: false, reason: "monthly_limit", remainingDaily, remainingMonthly };
