...
export function closureBlocks(c: ClosureInput): ClosureBlock[] {
  ...
  const total =
    c.pockets.reduce((a, p) => a + p.postedCents, 0) +
    (c.allowancePockets ?? []).reduce((a, p) => a + p.postedCents, 0);
  ...
  if (total > 0 && (!c.linkedBank || c.linkedBank.status !== "active" || !c.linkedBank.nameMatched))
    b.push("no_linked_bank");
  return b;
}

export interface ClosurePlan {
  cardsToCancel: string[];
  payoutCents: number;
  ledger?: Txn;
}
...
export function planClosure(c: ClosureInput, closureId: string): ClosurePlan {
  ...
  const lines = [
    ...c.pockets
      .filter((p) => p.postedCents > 0)
      .map((p) => dr("customer_deposits", p.postedCents, p.accountId)),
    ...(c.allowancePockets ?? [])
      .filter((p) => p.postedCents > 0)
      .map((p) => dr("family_allowance", p.postedCents, p.memberId)),
  ];
  const payoutCents = lines.reduce((a, l) => a + l.debit, 0);
  return {
    cardsToCancel: c.cards
      .filter((x) => x.status !== "canceled" && x.status !== "replaced")
      .map((x) => x.id),
    payoutCents,
    ledger:
      payoutCents > 0
        ? txn("closure_payout", [...lines, cr("closure_payout", payoutCents)], closureId)
        : undefined,
  };
}
