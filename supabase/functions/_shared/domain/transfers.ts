...
export function instantFee(amountCents: number, fees: FeeSchedule): number {
  return clamp(
    applyBps(amountCents, fees.instantTransferBps),
    fees.instantTransferMinCents,
    fees.instantTransferMaxCents,
  );
}
...
function commonChecks(
  ...
  if (!lim.ok)
    throw new TransferError(
      lim.reason!,
      `${lim.reason}: remaining today ${lim.remainingDaily}, month ${lim.remainingMonthly}`,
    );
...
export function planAchPush(
  ...
  if (p.sender.kyc === "frozen_legal" || (canMoveMoney(p.sender.kyc) && !canPayout(p.sender.kyc)))
    throw new TransferError("payout_blocked");
  ...
  if (inCoolingOff(p.bank, p.now, policy))
    throw new TransferError(
      "cooling_off",
      `withdrawals to a newly linked bank are allowed after ${policy.achOut.coolingOffHours}h`,
    );
...
export function planP2P(
  p: {
    transferId: string;
    amountCents: number;
    senderUserId: string;
    sender: SenderCtx;
    recipient: RecipientCtx | null;
    knownPayee: boolean;
  ...
  policy: MoneyPolicy,
  fees: FeeSchedule,
): { feeCents: number; ledger: Txn; newPayee: boolean } {
  if (!p.recipient || !canReceive(p.recipient.kyc) || p.recipient.accountStatus !== "open")
    throw new TransferError("recipient_unavailable");
  if (p.recipient.userId === p.senderUserId) throw new TransferError("self_transfer");
  if (
    p.amountCents < policy.p2p.minCents &&
    Number.isSafeInteger(p.amountCents) &&
    p.amountCents > 0
  )
    throw new TransferError("below_minimum");
  const feeCents = fees.p2pCents;
  commonChecks(p.amountCents, feeCents, p.sender, p.now, policy);
  const newPayee = !p.knownPayee;
  if (newPayee && policy.p2p.newPayeeStepUp && !p.stepUpVerified)
    throw new TransferError("step_up_required");
  return {
    feeCents,
    newPayee,
    ledger: txn(
...
export function planPocketMove(p: {
  transferId: string;
  ...
  return txn(
    "pocket_move",
    [
      dr("customer_deposits", p.amountCents, p.fromAccountId),
      cr("customer_deposits", p.amountCents, p.toAccountId),
    ],
    p.transferId,
  );
}
