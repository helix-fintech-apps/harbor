...
import { DEFAULT_FEES, DEFAULT_POLICY, type FeeSchedule, type MoneyPolicy } from "../_shared/domain/index.ts";
...
async function activeConfig(): Promise<{ policy: MoneyPolicy; fees: FeeSchedule }> {
  ...
  return { policy: (p?.[0]?.policy as MoneyPolicy) ?? DEFAULT_POLICY, fees: (f?.[0]?.schedule as FeeSchedule) ?? DEFAULT_FEES };
}

async function verifyStripeSignature(payload: string, header: string | null, secret: string | undefined): Promise<boolean> {
...
Deno.serve(async (req) => {
  ...
  if (path === "/webhooks/stripe" && req.method === "POST") {
    ...
    if (!(await verifyStripeSignature(raw, req.headers.get("stripe-signature"), env.STRIPE_WEBHOOK_SECRET))) return json(400, { error: { code: "bad_signature" } });
    const evt = JSON.parse(raw);
    const { error: dup } = await admin.from("provider_events").insert({ id: evt.id, provider: "stripe", type: evt.type });
    if (dup) return json(200, { received: true, duplicate: true });
    if (evt.type === "issuing_authorization.request") {
      const a = evt.data.object;
      const { data: card } = await admin.from("cards").select("id").eq("provider_card_id", a.card.id).single();
      const decision = card
        ? await service.authorizeCard(card.id, { amountCents: a.pending_request?.amount ?? a.amount, mcc: a.merchant_data?.category_code ?? "0000", merchant: a.merchant_data?.name ?? "Merchant", foreign: a.merchant_data?.country && a.merchant_data.country !== "US", providerAuthId: a.id })
        : { approved: false };
  ...
  if (token) {
    ...
    if (data.user) {
      ...
      caller = { userId: data.user.id, role: (prof?.role ?? "customer") as "customer" | "admin" | "support_agent" };
    }
  }
  // Fake-only test hooks (card network simulator) are disabled when a real issuer is configured.
  if (path.startsWith("/sim/") && providers.issuer.name !== "fake") return json(404, { error: { code: "no_route" } });

  let body: unknown = {};
  if (req.method !== "GET") { try { body = await req.json(); } catch { body = {}; } }
  try {
    const res = await route(service, {
      method: req.method, path, body, caller, idempotencyKey: req.headers.get("idempotency-key") ?? undefined,
