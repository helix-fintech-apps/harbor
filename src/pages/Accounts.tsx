  ...
  const move = async () => {
    ...
    setMoveRes(
      await call(
        "POST",
        "/transfers/pocket",
        { from, to, amountCents: dollarsToCents(amount) },
        newIdempotencyKey(),
      ),
    );
    reload();
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <Badge tone={kycTone(p.kycState)} testId="kyc-state">
          {p.kycState.replace("_", " ")}
        </Badge>
        <Badge tone="blue" testId="tier">
          {p.tier}
        </Badge>
      </div>
      {p.kycState !== "approved" && (
        <Section title="Verify your identity" testId="kyc-panel">
          <p className="muted mb-3">
            {p.kycState === "unverified" &&
              "We need to verify your identity before you can move money."}
            {p.kycState === "pending" &&
              "Your verification is still processing. You can check again."}
            ...
            {(p.kycState === "suspended" || p.kycState === "frozen_legal") &&
          ...
          {(p.kycState === "unverified" || p.kycState === "pending") && (
            <button
              ...
              onClick={async () => {
                setKycRes(await call("POST", "/kyc/start"));
                reload();
              }}
            >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {me.accounts.map((a: any) => (
          <div key={a.id} className="card" data-testid={`account-${a.kind}`}>
            <div className="flex items-center justify-between">
              ...
              <Badge
                tone={a.status === "open" ? "green" : "red"}
                testId={`account-${a.kind}-status`}
              >
                {a.status}
              </Badge>
            </div>
            <p className="muted">
              Acct {a.accountNumber} · Routing {a.routingNumber}
            </p>
            <div className="mt-3 grid grid-cols-3 gap-2">
              <div>
                ...
                <Money
                  ...
                  testId={`${a.kind}-available`}
                  className="text-2xl font-semibold"
                />
              </div>
              <div>
                <p className="label">Posted</p>
                <Money cents={a.postedCents} testId={`${a.kind}-posted`} className="text-lg" />
              </div>
              <div>
        ...
        {me.accounts.length === 0 && (
          <p className="muted" data-testid="no-accounts">
            Accounts open once your identity is verified.
          </p>
        )}
      </div>
      {me.accounts.length > 0 && (
        <Section title="Move between pockets" testId="pocket-move">
          <div className="flex flex-wrap items-end gap-2">
            ...
            <input
              ...
              data-testid="pocket-amount"
            />
            <button className="btn" onClick={move} data-testid="pocket-submit">
              Move
            </button>
          </div>
      ...
      <Section title="Limits" testId="limits">
        <p className="muted">
          Daily transfers out <Money cents={me.limits.dailyTransferOutCents} /> · Monthly{" "}
          <Money cents={me.limits.monthlyTransferOutCents} /> · Daily card spend{" "}
          <Money cents={me.limits.dailyCardSpendCents} /> · Monthly card spend{" "}
          <Money cents={me.limits.monthlyCardSpendCents} />
        </p>
      </Section>
      <Section
        ...
        testId="activity"
        right={
          <Link to="/statements" className="text-sm text-harbor-700">
            Statements →
          </Link>
        }
      >
          ...
          <tbody>
            {me.transfers.map((t: any) => (
              <tr key={t.id} data-testid="activity-row">
                ...
                <td>
                  {t.kind}
                  {t.speed ? ` (${t.speed})` : ""}
                  {t.counterparty_user_id === p.id ? " received" : ""}
                </td>
                <td>
                  <Badge
                    tone={
                      t.status === "returned" ? "red" : t.status === "pending" ? "amber" : "green"
                    }
                  >
                    {t.status}
            ...
            {me.authorizations.map((a: any) => (
              <tr key={a.id} data-testid="card-activity-row">
                ...
                <td className="text-right">
                  <Money cents={a.status === "captured" ? a.captured_cents : a.amount_cents} />
                </td>
                <td className="text-right">
                  <Money cents={a.fee_cents} />
                </td>
              </tr>
            ))}
          </tbody>
