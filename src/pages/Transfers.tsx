...
export default function Transfers() {
  ...
  return (
    <div className="space-y-5">
      ...
      {chk && (
        <p className="muted">
          Checking available: <Money cents={chk.availableCents} testId="transfers-available" />
        </p>
      )}
      <Section title="Add money (ACH from linked bank)" testId="ach-in">
        <div className="flex flex-wrap items-end gap-2">
          ...
          <input
            ...
            data-testid="ach-in-amount"
          />
          <button
            className="btn"
            data-testid="ach-in-submit"
            onClick={async () => {
              setInRes(
                await call(
                  "POST",
                  "/transfers/ach-in",
                  { bankId: inBank, amountCents: dollarsToCents(inAmt) },
                  inKey,
                ),
              );
              reload();
        ...
        <p className="muted mt-2">
          Deposits post right away but stay on hold for {terms?.policy.achIn.holdBusinessDays ?? 3}{" "}
          business days until the transfer settles.
        </p>
      ...
      <Section title="Withdraw to linked bank" testId="ach-out">
        <div className="flex flex-wrap items-end gap-2">
          <select
            className="input w-72"
            value={outBank}
            onChange={(e) => setOutBank(e.target.value)}
            data-testid="ach-out-bank"
          >
            <option value="">Select bank…</option>
            {banks.map((b) => (
              <option key={b.id} value={b.id}>
          ...
          <label className="flex items-center gap-1 text-sm">
            <input
              ...
              data-testid="speed-instant"
            />{" "}
            Instant
          </label>
          <button
            className="btn"
            data-testid="ach-out-submit"
            onClick={async () => {
              setOutRes(
                await call(
                  "POST",
                  "/transfers/ach-out",
                  { bankId: outBank, amountCents: outCents, speed },
                  outKey,
                ),
              );
              reload();
        ...
        <p className="mt-2 text-sm" data-testid="ach-out-fee-quote">
          Fee: <Money cents={fee} testId="ach-out-fee" /> · Total debit:{" "}
          <Money cents={(Number.isFinite(outCents) ? outCents : 0) + fee} testId="ach-out-total" />
        </p>
      ...
      <Section title="Send to a Harbor member" testId="p2p">
          ...
          <button
            ...
            onClick={async () => {
              const r = await call(
                ...
                {
                  recipientEmail: to,
                  amountCents: dollarsToCents(p2pAmt),
                  stepUpCode: code || undefined,
                },
                p2pKey,
              );
              if (r.error?.code === "step_up_required") setNeedCode(true);
              else if (r.ok) {
                setNeedCode(false);
                setCode("");
              }
        ...
        {needCode && (
          <p className="mt-2 text-sm text-amber-700" data-testid="p2p-stepup-prompt">
            New recipient: enter the verification code we sent you (demo: 000000).
          </p>
        )}
