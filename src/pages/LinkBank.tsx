  ...
  return (
    <div className="space-y-5">
      ...
      <Section
        ...
        testId="banks"
        right={
          <button className="btn" onClick={startLink} data-testid="link-bank-start">
            Link a bank
          </button>
        }
      >
          ...
          <tbody>
            {me.banks.map((b: any) => (
              <tr key={b.id} data-testid="bank-row">
                <td>{b.institution}</td>
                <td>••••{b.mask}</td>
                <td>{b.ownerNames.join(", ")}</td>
                <td>
                  <Badge tone={b.nameMatched ? "green" : "red"} testId="bank-name-match">
                ...
                <td>
                  <button
                    ...
                    data-testid="bank-remove"
                    onClick={async () => {
                      await call("POST", `/banks/${b.id}/remove`);
                      reload();
                    }}
                  >
                    Remove
        ...
        {open && (
          <div
            className="mt-4 rounded-xl border border-harbor-100 bg-harbor-50 p-4"
            data-testid="plaid-modal"
          >
            <p className="mb-2 text-sm font-medium">
              {MODE === "demo" ? "Plaid (fake) — select your bank" : "Plaid sandbox"}
            </p>
            <div className="grid gap-2 md:grid-cols-3">
              ...
              <div>
                <label className="label">Account owner name (as the bank reports it)</label>
                <input
                  className="input"
                  value={owner}
                  onChange={(e) => setOwner(e.target.value)}
                  data-testid="plaid-owner"
                />
              </div>
              <div className="flex items-end gap-2">
                <button className="btn" onClick={finish} data-testid="plaid-continue">
                  Continue
                </button>
      ...
      <Section title="Switch your direct deposit" testId="direct-deposit">
        <p className="muted mb-3">
          We'll record your switch form. Give it to your employer; Harbor does not contact
          employers.
        </p>
        <div className="grid gap-2 md:grid-cols-4">
          <input
            ...
            value={dd.employerName}
            onChange={(e) => setDd({ ...dd, employerName: e.target.value })}
            data-testid="dd-employer"
          />
          <input
            className="input"
            placeholder="Percent of paycheck"
            value={dd.percent}
            onChange={(e) => setDd({ ...dd, percent: e.target.value })}
            data-testid="dd-percent"
          />
          <input
            className="input"
            placeholder="Signature (legal name)"
            value={dd.signatureName}
            onChange={(e) => setDd({ ...dd, signatureName: e.target.value })}
            data-testid="dd-signature"
          />
          <button
            className="btn"
            ...
            onClick={async () =>
              setDdRes(
                await call("POST", "/direct-deposit", {
                  employerName: dd.employerName,
                  allocation: { kind: "percent", percent: Number(dd.percent) },
                  signatureName: dd.signatureName,
                }),
              )
            }
          >
