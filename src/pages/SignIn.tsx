  ...
  return (
    <div className="mx-auto max-w-md space-y-4">
      <form className="card space-y-3" onSubmit={submit} data-testid="signin-form">
        ...
        {err && (
          <p className="text-sm text-red-600" data-testid="signin-error">
            {err}
          </p>
        )}
        <button className="btn w-full" data-testid="signin-submit">
          Sign in
        </button>
      </form>
      {MODE === "demo" && (
        <div className="card space-y-2" data-testid="demo-users">
          <p className="muted">
            Demo users (password <code>{DEMO_PASSWORD}</code>):
          </p>
          <div className="flex flex-wrap gap-2">
            {DEMO_USERS.map((u) => (
              <button
                ...
                onClick={() => submit(undefined, u.email, DEMO_PASSWORD)}
              >
                {u.legalName}{" "}
                <span className="ml-1 text-xs text-slate-500">
                  ({u.role === "customer" ? u.email.split("@")[0] : u.role})
                </span>
              </button>
            ))}
          </div>
