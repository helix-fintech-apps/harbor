      ...
      <Section title="Dispute a card transaction" testId="dispute-form">
        <div className="grid gap-2 md:grid-cols-4">
          <select
            ...
            data-testid="dispute-auth"
          >
            <option value="">Transaction…</option>
            {posted.map((a: any) => (
              <option key={a.id} value={a.id}>
                {a.merchant} — {formatCents(a.captured_cents - a.refunded_cents)}
              </option>
            ))}
          </select>
          <input
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th>Amount</th>
              <th>Status</th>
              <th>Provisional credit</th>
              <th>Credit due by</th>
              <th>Decision due by</th>
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={d.id} data-testid="dispute-row">
                ...
                <td>
                  <Badge
                    ...
                    testId="dispute-status"
                  >
                    {d.status.replace("_", " ")}
                  </Badge>
                </td>
                <td>
                  <Money cents={d.provisional_credit_cents} testId="dispute-provisional" />
                </td>
