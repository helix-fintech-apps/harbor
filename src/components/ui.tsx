...
export function Money({
  ...
  return (
    <span
      data-testid={testId}
      data-cents={cents}
      className={`${cents < 0 ? "text-red-600" : ""} ${className ?? ""}`}
    >
      {formatCents(cents)}
    </span>
  );
...
export function Badge({
  ...
  const tones = {
    ...
    blue: "bg-harbor-100 text-harbor-700",
  };
  return (
    <span
      data-testid={testId}
...
export function kycTone(s: string) {
  return s === "approved"
    ? "green"
    : s === "rejected" || s === "frozen_legal"
      ? "red"
      : s === "needs_review" || s === "suspended"
        ? "amber"
        : "slate";
}
